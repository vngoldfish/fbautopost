...
APIRouter for AI-powered Comment Reply Suggestions (sales, discount, friendly tones).
"""

import random
from fastapi import APIRouter
from app.models.ai import AiSuggestReplyRequest, AiSuggestReplyResponse

router = APIRouter(prefix="/api/ai", tags=["AI"])

@router.post("/suggest-reply", response_model=AiSuggestReplyResponse)
def suggest_reply(payload: AiSuggestReplyRequest):
    """Generates intelligent context-aware comment replies."""
    comment_text = (payload.commentText or "").strip()
    post_content = (payload.postContent or "").strip()
